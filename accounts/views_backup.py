...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            check_user_username = User.objects.get(username=username)

            if check_user_username is not None:
                print("username is already registered. please replace your username!!!")
                print(check_user_username)
                print(str(type(check_user_username)))
                messages.error(request, '사용 중인 아이디 입니다. 다시 시도해 주세요.')
                args = {'form': form}
                return render(request, 'accounts/register_form.html', args)

        except:
            print("Not registered username! You can user your email or id!!!")
            pass

        try:
            check_user_email = User.objects.get(email=email)
            if check_user_email is not None:
                print("email is already registered. please replace your email!!!")
                print(check_user_email)
                print(str(type(check_user_email)))
                messages.error(request, '사용 중인 이메일이 있습니다. 다시 시도해 주세요.')
                args = {'form': form}

                return render(request, 'accounts/register_form.html', args)

        except:
            print("Not registered email address! You can user your email or id!!!")
            pass
